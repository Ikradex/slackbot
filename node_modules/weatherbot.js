...
WeatherBot.prototype.getCurrent = function (res, cityName, countryName) {
	var botPayload = {
		text: " ",
		attachments: []
	};
...
WeatherBot.prototype._buildAttachments = function (reportJson, reportType) {
	...
	if(reportType == WeatherBot.REPORT_CURRENT) {
		...
		attachments.push({
			...
			"fields": [{
				"title": "Temp",
				"value": vsprintf("(Curr) %s&deg;, (Min) %s&deg;, (Max) %s&deg;", [dayJson.temp.curr, dayJson.temp.min, dayJson.temp.max]),
				"short": true
			}, {
				"title": "Humidity",
