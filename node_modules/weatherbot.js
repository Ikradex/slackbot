...
WeatherBot.prototype.handleRequest = function (req, res, next) {
	...
	if (argsStr.trim()) {
		...
		case "today":
		case "now":
			this.get(res, cityName, countryName, WeatherBot.REPORT_CURRENT);
			break;
		case "tomorrow":
...
WeatherBot.prototype.get = function (res, cityName, countryName, reportType) {
	var botPayload = {
		text: " ",
		attachments: []
	};
...
WeatherBot.prototype._buildAttachments = function (reportJson, reportType) {
	...
	if(reportType == WeatherBot.REPORT_CURRENT) {
		...
		reportJson.days.forEach(function(day) {
			var date = new Date();
			date.setTime(dayJson.dt);
